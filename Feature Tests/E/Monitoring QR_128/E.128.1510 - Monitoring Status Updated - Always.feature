Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
          This feature tests the option 'Always whenever any field is updated'
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1510" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/E1281500.xml", and clicking the "Create Project" button
    ...
    And I select "accepted: value updated" in the dropdown field in column "Response" for the field "ptname"
    And I select "accepted: can't resolve" in the dropdown field in column "Response" for the field "text2"
    And I enter "Response1" in the column "Response" for the field "text2"
    And I select "rejected: query is in error" in the dropdown field in column "Response" for the field "notesbox"
    ...
    And I should see a table header and rows containing the following values in a table:
      ...
      | ptname            | Paul        | accepted-updated                  |
      | text2             |             | accepted-unresolvable [Response1] |
      | notesbox          |             | rejected-error [Response2]        |
