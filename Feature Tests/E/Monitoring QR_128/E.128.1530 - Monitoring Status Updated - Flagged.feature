Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
          This feature tests the option 'When the updated field is flagged'
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1530" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/E1281500.xml", and clicking the "Create Project" button
