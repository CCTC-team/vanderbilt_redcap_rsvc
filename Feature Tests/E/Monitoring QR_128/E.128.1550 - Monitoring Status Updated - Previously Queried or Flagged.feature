Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
          This feature tests the option 'When the updated field has been previously queried or the field is flagged'
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1550" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/E1281500.xml", and clicking the "Create Project" button
    ...
    And I select "accepted: value updated" in the dropdown field in column "Response" for the field "text2"
    ...
    When I locate the bubble for the "Text Validation" instrument on event "Event 1" for record ID "1-1" and click on the bubble
    Then I should see "Showing queried fields only. Waiting for responses."
    And I select "accepted: value updated" in the dropdown field in column "Response" for the field "ptname_v2"
    When I click on the button labeled "Send response"
    Then I should NOT see the monitoring table
    And I logout
