Feature: E.128.1500 - The system shall automatically update the monitoring status to 'Requires verification due to data change' if the current status is 'Verification complete' based on the selected option using Monitoring QR external module.
          This feature tests the option 'Never'
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.128.1520" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/E1281500.xml", and clicking the "Create Project" button
    ...
    And I should see a table header and row containing the following values in a table:
      | Timestamp        | Query status | Field [label] | Query  | Response [comment] |
      | mm/dd/yyyy hh:mm | OPEN         | ptname [Name]	| Query1 | accepted-updated   |
      | mm/dd/yyyy hh:mm | OPEN         | ptname [Name]	| Query1 |                    |

    When I click on the button labeled "Hide history"
    Then I should NOT see "Monitor query history"
    And I should NOT see "Query1"
